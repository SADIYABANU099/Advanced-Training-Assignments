...
public class ProductManagementDAO {
    ...
    public Product getProductByid(String productId)
    {
        ...
        {
            Connection conn = DButil.getConnection();
            PreparedStatement ps = conn.prepareStatement("SELECT * FROM productData WHERE productId = ?");
    ...
    public int updateProduct(Product product)
    {
        ...
        {
            Connection conn = DButil.getConnection();
            PreparedStatement ps = conn.prepareStatement("UPDATE productData SET productName=?, productPrice=? WHERE productId=?");
            //set parameters of query here but using the values for the product object
    ...
    public int deleteProduct(String productId)
    {
        ...
        {
            Connection conn = DButil.getConnection();
            PreparedStatement ps = conn.prepareStatement("DELETE FROM productData where productId = ?");
            //set parameters of query here but using the values for the product object
            ps.setString(1, productId);
            status = ps.executeUpdate();  //if successful status should return 1
